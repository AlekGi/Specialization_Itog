...
public class View implements IView {
    ...
    public boolean showAddAnimalDialog() {
        ...
        while (true) {
            ...
            try (counter){
                ...
                counter.add();
                return controller.createAnimal(parsedData.name, parsedData.birthDate, parsedData.animalType);
    ...
    public void showAnimalInfo(Animal animal) {
        while (true) {
            ...
            AddMenuCommand code = showAddCommandMenu(animal);
            switch (code) {
                case ADD_SKILL -> {
                    boolean result = showAddCommandDialog(animal);
    ...
    @Override
    public AddMenuCommand showAddCommandMenu(Animal animal) {
        String menu = String.format(
                "%d-[%s]\t%d-[%s]\n",
                AddMenuCommand.ADD_SKILL.ordinal(), AddMenuCommand.ADD_SKILL.getTag(),
                AddMenuCommand.EXIT.ordinal(), AddMenuCommand.EXIT.getTag()) ;
        printLineWithSymbol("=", SIZE_LINE);
        printColorLine("Доступные действия:", GREEN_COLOR);
        printColorLine(menu, GREEN_COLOR);
        while (true) {
            try {
                System.out.printf("%s (%d - %d): ", "Выберите действие: ", AddMenuCommand.ADD_SKILL.ordinal(),
                        AddMenuCommand.EXIT.ordinal());
                scanner = new Scanner(System.in);
                return AddMenuCommand.values()[scanner.nextInt()];
            } catch (ArrayIndexOutOfBoundsException | InputMismatchException e) {
                printColorLine("Некоректное действие!", RED_COLOR);
            }
        }
    ...
    @Override
    public boolean showAddCommandDialog(Animal animal) {
        ...
        if (skillData.length == 1) {
            animal.learnCommand(new Command(skillData[0]));
        } else if (skillData.length == 2) {
            animal.learnCommand(new Command(skillData[0], skillData[1]));
        } else {
    ...
    public int showRemoveAnimalDialog() {
        ...
        while (true) {
            try {
                ...
                return controller.removeAnimal(animal);
            } catch (IndexOutOfBoundsException e) {
                System.out.println("Запись с таким номером отсутствует");
            }
        }
